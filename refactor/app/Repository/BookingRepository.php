...
class BookingRepository extends BaseRepository
{
    ...
    public function getUsersJobsHistory($user_id, Request $request)
    {
        ...
        $noramlJobs = array();
        if ($cuser && $cuser->is('customer')) {
            $jobs = $cuser->jobs()->with('user.userMeta', 'user.average', 'translatorJobRel.user.average', 'language', 'feedback', 'distance')->whereIn('status', ['completed', 'withdrawbefore24', 'withdrawafter24', 'timedout'])->orderBy('due', 'desc')->paginate(config("paginate.items_per_page"));
            $usertype = 'customer';
    ...
    public function sendSMSNotificationToTranslator($job)
    {
        ...
        foreach ($translators as $translator) {
            ...
            $status = SendSMSHelper::send(config('roles.SMS_NUMBER'), $translator->mobile, $message);
    ...
    public function sendPushNotificationToSpecificUsers($users, $job_id, $data, $msg_text, $is_need_delay)
    {
        ...
        if (config('roles.APP_ENV') == 'prod') {
            $onesignalAppID = config('roles.prodOnesignalAppID');
            $onesignalRestAuthKey = sprintf("Authorization: Basic %s", config('roles.prodOnesignalApiKey'));
        } else {
            $onesignalAppID = config('roles.devOnesignalAppID');
            $onesignalRestAuthKey = sprintf("Authorization: Basic %s", config('roles.devOnesignalApiKey'));
        }
    ...
    public function acceptJob($data, $user)
    {
        ...
        $adminemail = config('roles.admin_email');
        $adminSenderEmail = config('roles.admin_sender_email');
    ...
    public function parseRequest(Request $request)
    {
        ...
        // Perform validation or sanitization here, if needed
    ...
    {
        // Apply all your filters to the query based on the $filters array
        // For example:
        ...
        if ( count($requestdata['translator_email'])) {
        ...
        if ( $requestdata['filter_timetype'] ?? '') {
            if ( $requestdata['from'] ?? '') {
    ...
    {
        $filters = $this->parseRequest($request);
        ...
        // Apply other query manipulations here

        if (!($cuser && $cuser->user_type == env('SUPERADMIN_ROLE_ID'))) {
            if ($consumer_type == config('roles.consumer_type_rws')) {
                $query->where('job_type', '=', config('roles.job_type_rws'));
            } else {
                $query->where('job_type', '=', config('roles.job_type_unpaid'));
            }
        }
        if ($limit == 'all') {
            $allJobs = $query->get();
        } else {
            $allJobs = $query->paginate(config("paginate.items_per_page"));
        }
    ...
    public function alerts()
    {
        ...
        if ($cuser && $cuser->is('superadmin')) {
            ...
            $allJobs->orderBy('jobs.created_at', 'desc');
            $allJobs = $allJobs->paginate(config("paginate.items_per_page"));
        }
    ...
    {
        $throttles = Throttles::where('ignore', 0)->with('user')->paginate(config("paginate.items_per_page"));
